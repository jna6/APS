package Hash.person;// Следните класи веќе се импортирани, не е дозволено копирање на класи овде, директно користејте ги како кога се достапни во други локални фајлови:
...
class Project {
    ...
    public Project(int workTime, int hourSalary) {
...
public class Solution {
    public static void main(String[] args) {
        ...
        for (int i=0;i<n;i++){
            ...
            int hourSalary=scanner.nextInt();

            Person person = new Person(name,age);
            Project newProject= new Project(workTime,hourSalary);

            SLLNode<MapEntry<Person,Project>> curr= table.search(person);
            if (curr==null){
                table.insert(person,newProject);
            }else {
                Project exsistingProject= curr.element.value;
                if (newProject.getTotalSalary()> exsistingProject.getTotalSalary()){
                    table.insert(person,newProject);
                }
            }
        }
