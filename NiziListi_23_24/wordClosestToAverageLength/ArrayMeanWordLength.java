package NiziListi_23_24.wordClosestToAverageLength;
...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        float closestWordDifference= Math.abs(closestWord.length()-avg);

        for (int i=0;i< arr.getSize();i++){
            String str= arr.get(i);
            float difference= Math.abs(str.length() -avg);
            if (difference<closestWordDifference){
                closestWord=str;
                closestWordDifference=difference;
            }
            if (difference==closestWordDifference){
                if (str.length()>closestWord.length()){
                    closestWord=str;
                }
            }
        }
